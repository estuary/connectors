...
func (b *AzureBlobBucket) URI(key string) string {
	// This is not a scheme-specific URI like S3 and GCS use, where they start
	// with `s3://` or `gs://`, but rather a full HTTPS URL, since that is all
	// Azure officially supports.
