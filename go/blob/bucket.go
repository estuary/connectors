...
type Bucket interface {
	...
	// URI creates an identifier from a file key, usually by prepending the URI
	...
	// List lists the items in the bucket. It only yields actual objects; never
	// "directory" or prefix items.
...
type ObjectInfo struct {
	// Key is the path of the object. It does not include the bucket name.
	Key string
}
