...
func (c *Capture) startFlowctl(args ...string) (*flowctlCmd, error) {
	cmd := exec.Command("flowctl", append([]string{"--profile=testing"}, args...)...)
	cmd.Dir = c.runRoot
	cmd.Env = append(os.Environ(), "NO_COLOR=1", "LOG_FORMAT=minimal")
...
func (c *Capture) Spec() (json.RawMessage, error) {
	...
	fc, err := c.startFlowctl("raw", "spec", "--source", path)
	if err != nil {
		return nil, err
	}
...
func (c *Capture) DiscoverRaw() ([]json.RawMessage, error) {
	...
	fc, err := c.startFlowctl("raw", "discover", "--source", path, "-o", "json", "--emit-raw")
	if err != nil {
		return nil, err
	}
...
func (c *Capture) Discover() (json.RawMessage, error) {
	...
	fc, err := c.startFlowctl("raw", "discover", "--source", path)
	if err != nil {
		return nil, err
	}
	...
	fc, err = c.startFlowctl("raw", "bundle", "--source", path, "-o", "json")
	if err != nil {
		return nil, err
	}
