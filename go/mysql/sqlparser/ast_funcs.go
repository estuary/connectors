...
import (
	...
	"github.com/estuary/connectors/go/mysql/sqlparser/internal/sqltypes"
)
...
func SQLTypeToQueryType(typeName string, unsigned bool) sqltypes.Type {
	...
	if node != nil {
		for _, comment := range node.comments {
			if strings.HasPrefix(comment, queryOptimizerPrefix) {
				if hasQueryHint {
					return nil, fmt.Errorf("Must have only one query hint")
				}
				hasQueryHint = true
				idx := strings.Index(comment, "*/")
				if idx == -1 {
					return nil, fmt.Errorf("Query hint comment is malformed")
				}
...
func (node *AliasedTableExpr) TableName() (TableName, error) {
	...
	if !ok {
		return TableName{}, fmt.Errorf("BUG: the AST has changed, this should not be possible")
	}
...
func (node *AliasedTableExpr) TableNameString() string {
	...
	tableName, ok := node.Expr.(TableName)
	if !ok {
		panic("derived table should have an alias, this should not be possible")
	}
...
func setFuncArgs(aggr AggrFunc, exprs Exprs, name string) error {
	if len(exprs) != 1 {
		return fmt.Errorf("%s: incorrect parameter count, expected exactly one argument", name)
	}
	aggr.SetArg(exprs[0])
	return nil
}
...
// IsValue returns true if the expression is a bind variable or a string,
// hex, or integer literal. Used when formatting to decide whether an
// expression is atomic and therefore never needs surrounding parentheses.
// Inlined from the upstream analyzer.go (dropped).
