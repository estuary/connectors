...
// This file was extracted from vitess.io/vitess/go/vt/sqlparser (v0.21.3) and
// modified by Estuary Technologies, Inc. See README.md and origin.json for the
...
// defaultMySQLVersion is the MySQL server version assumed when none is
// configured. Inlined from the upstream vitess.io/vitess/go/mysql/config
// package, which only contributed this single constant.
const defaultMySQLVersion = "8.0.30"

// BindVars is the set of bind variable names found while parsing a query.
// Upstream this lived in normalizer.go (dropped); the parser still records
// names as it scans, and Parse2 returns them.
...
func (p *Parser) Parse2(sql string) (Statement, BindVars, error) {
	...
	if yyParsePooled(tokenizer) != 0 {
		if tokenizer.partialDDL != nil {
			...
			// The DDL did not fully parse, but the grammar recovered a partial
			// statement. We do not log here (upstream emitted a glog warning):
			// callers detect the gap via FullyParsed on the returned statement
		...
		return nil, nil, errors.New(tokenizer.LastError.Error())
	}
...
func ConvertMySQLVersionToCommentVersion(version string) (string, error) {
	...
	if idx == 0 {
		return "", fmt.Errorf("MySQL version not correctly setup - %s", version)
	}
...
var ErrEmpty = errors.New("Query was empty")
...
func New(opts Options) (*Parser, error) {
	if opts.MySQLServerVersion == "" {
		opts.MySQLServerVersion = defaultMySQLVersion
	}
...
func NewTestParser() *Parser {
	convVersion, err := ConvertMySQLVersionToCommentVersion(defaultMySQLVersion)
	if err != nil {
		panic(err)
	}
	return &Parser{
