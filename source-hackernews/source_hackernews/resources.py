...
def items(http: HTTPSession):
    def open(
        ...
        async def fetch_items_batched(log, start_cursor, log_cutoff) -> AsyncGenerator[Item | int, None]:
            async def fetch_with_retry(item_id: int, max_retries: int = 3) -> Optional[Item]:
                for attempt in range(max_retries):
                    try:
                        ...
                        if attempt == max_retries - 1:
                            log.error(f"Failed to fetch item {item_id} after {max_retries} attempts: {e}")
                            return None
            ...
            async def _batches_gen() -> AsyncGenerator[Awaitable[List[Item]], None]:
                total_items = 100  # Initial batch size
                batch_size = min(20, max(5, int(total_items / 10)))  # Dynamic batch size
                for batch_it in itertools.batched(range(start_cursor, start_cursor + total_items), batch_size):
                    yield _do_batch_fetch(list(batch_it))

            total = 100
            count = 0
            concurrency = min(10, max(3, int(total / 20)))  # Adaptive concurrency
