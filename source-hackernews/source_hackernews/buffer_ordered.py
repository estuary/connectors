...
class BufferWork(Generic[T]):
    aw: Awaitable[T]
    result: asyncio.Queue[Union[T, None]]
    next: asyncio.Future[Optional["BufferWork[T]"]]
...
async def managed_buffer_ordered(
    ...
    """
    work: asyncio.Queue[Union[BufferWork[T], None]] = asyncio.Queue(1)
    tasks = []
    producer_task = None
    worker_tasks = []

    async def _producer(next_future: asyncio.Future[Union[BufferWork[T], None]]):
        current = next_future
        try:
            async for aw in aws:
                this_next: asyncio.Future[Union[BufferWork[T], None]] = asyncio.Future()
    ...
    async def _run():
        next_future = asyncio.Future[Union[BufferWork[T], None]]()
        try:
