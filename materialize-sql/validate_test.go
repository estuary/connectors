...
func TestValidateMigrations(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			validator := boilerplate.NewValidator(testConstrainter{featureFlags: tt.featureFlags}, is, tt.maxFieldLength, true, tt.featureFlags)
...
func (tc testConstrainter) NewConstraints(p *pf.Projection, deltaUpdates bool, _ json.RawMessage) (*pm.Response_Validated_Constraint, error) {
	...
	case !deltaUpdates && !tc.featureFlags["flow_document"] && strings.Count(p.Ptr, "/") == 1 && p.Inference.Exists == pf.Inference_MUST:
