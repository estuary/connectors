...
// UUID for the runtime to compare against a collection's backfill markers. It is
// nil when no such projection is materialized, in which case the binding cannot
// support truncations.
//
// Note that /_meta/uuid is not a root-level pointer, so it is never among
