...
func TestMySQLConfig(t *testing.T) {
	...
	require.Equal(t, "youser:shmassword@tcp(post.toast:1234)/namegame?clientFoundRows=true", uri)

	var noPort = validConfig
	noPort.Address = "post.toast"
	require.NoError(t, noPort.Validate())
	uri = noPort.ToURI()
	require.Equal(t, "youser:shmassword@tcp(post.toast:3306)/namegame?clientFoundRows=true", uri)
