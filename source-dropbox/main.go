...
func (dbStore *dropboxStore) List(ctx context.Context, query filesource.Query) (filesource.Listing, error) {
	// The Dropbox API returns pages of entries sorted by their modification date and time. However for
	// the connector to work with the `filesource` package correctly, the listing returned by `List` must
	// contain all entries sorted in lexicographic, monotonic order. To achieve this, we paginate through
	// all entries, convert them to `filesource.ObjectInfo`s, and sort them based on their path.

	// Get the first page of entries at the specified path from Dropbox.
	...
	// Get the remaining pages of entries.
	for f.HasMore {
	...
	for _, entry := range entries {
		...
		// Only add files and folders that are lexicographically
		// greater than or equal to the query's StartAt property.
