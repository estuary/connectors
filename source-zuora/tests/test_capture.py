...
from source_zuora.models import ZuoraDocument
...
class FakeManager:
    """Mimics ExportManager.export_rows: parses the query's UpdatedDate window,
    yields matching records sorted ascending (as ORDER BY UpdatedDate would), and
    optionally raises ExportTooLargeError for windows wider than too_large_over.
    """
    ...
    def __init__(
        ...
        # When set, output UpdatedDate values carry milliseconds, as Zuora's
        # export does. The query filter is still parsed at whole-second precision
        # (as Export ZOQL only accepts second literals), so a sub-second record
        # sits above a second-granular >= bound — the realistic mismatch the
    ...
    async def export_rows(self, query: str):
        ...
        lo = re.search(r"UpdatedDate >= '([^']+)'", query)
        hi = re.search(r"UpdatedDate < '([^']+)'", query)
        ...
        for rid, dt in self.records:
            if (start is None or dt >= start) and (end is None or dt < end):
                yield {"Id": rid, "UpdatedDate": dt.astimezone(UTC).strftime(out_fmt)}
...
def test_build_query_bounds_add_where_and_order_by():
    q = api.build_query(
        "Account",
        ["Id"],
        updated_after=datetime(2020, 1, 1, tzinfo=UTC),
        updated_before=datetime(2020, 2, 1, tzinfo=UTC),
    )
    assert q == (
...
def test_build_query_normalizes_non_utc_bounds_to_utc():
    ...
    q = api.build_query(
        "Account", ["Id"], updated_after=datetime(2020, 1, 1, tzinfo=plus_ten)
    )
...
async def _run_changes(manager, cursor):
    return await _collect(
        api.fetch_changes("Account", ["Id", "UpdatedDate"], manager, _LOG, cursor)
    )
...
async def _run_page(manager, start_date, page, cutoff):
    return await _collect(
        api.fetch_page("Account", ["Id", "UpdatedDate"], manager, start_date, _LOG, page, cutoff)
    )
