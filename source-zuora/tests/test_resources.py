...
from source_zuora.models import EndpointConfig, ZuoraDocument
...
async def test_describe_one_success_returns_described_object():
    ...
    with patch("source_zuora.api.describe_object", fake):
        result = await resources._describe_one(
            "Account", "u", AsyncMock(), _LOG, asyncio.Semaphore(1)
        )
...
async def test_describe_one_http_error_is_skipped():
    ...
    with patch("source_zuora.api.describe_object", fake):
        assert await resources._describe_one(
            "Account", "u", AsyncMock(), _LOG, asyncio.Semaphore(1)
        ) is None
...
def test_incremental_resource_shape_and_boundary_ownership():
    ...
    r = resources._incremental_resource(
        "Account", ["Id", "UpdatedDate"], object(), start, cutoff
    )
    assert r.key == ["/Id"]
    assert r.model is ZuoraDocument
...
async def test_all_resources_enumerates_catalog_and_dispatches_by_updated_date():
    ...
    with patch("source_zuora.api.describe_object", fake_describe), patch(
        "source_zuora.api.discover_object_names", fake_catalog
    ), patch.object(resources, "_setup_http_auth"):
...
async def test_validate_credentials_success():
    ...
    with patch("source_zuora.api.discover_object_names", ok), patch.object(
        resources, "_setup_http_auth"
    ):
...
async def test_validate_credentials_401_reports_auth_failure():
    ...
    with patch("source_zuora.api.discover_object_names", unauthorized), patch.object(
        resources, "_setup_http_auth"
    ):
...
async def test_validate_credentials_other_http_error_is_generic():
    ...
    with patch("source_zuora.api.discover_object_names", server_error), patch.object(
        resources, "_setup_http_auth"
    ):
        with pytest.raises(ValidationError) as exc:
            await resources.validate_credentials(_LOG, AsyncMock(), _config())
...
async def test_validate_credentials_non_http_error_is_generic():
    ...
    with patch("source_zuora.api.discover_object_names", broken), patch.object(
        resources, "_setup_http_auth"
    ):
        with pytest.raises(ValidationError) as exc:
            await resources.validate_credentials(_LOG, AsyncMock(), _config())
...
def test_setup_http_auth_configures_oauth_token_source():
    ...
    resources._setup_http_auth(http, _config())
