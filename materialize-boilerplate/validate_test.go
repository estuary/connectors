...
func TestValidate(t *testing.T) {
	...
	t.Run("cannot add new root document without backfill", func(t *testing.T) {
		...
		// constraint, so the control plane fails the build with a clear error instead of
		// silently continuing to materialize without a document column.
	...
	t.Run("new binding to existing table with incompatible document column", func(t *testing.T) {
		// Regression test for the "silent INSERT-only" bug: a new binding attached to an
		// existing table whose document column has an incompatible type must report both
		// LOCATION_REQUIRED and INCOMPATIBLE for the document field. A lone INCOMPATIBLE
		// constraint is dropped by the control plane for new bindings (no live spec), which
		// silently degrades the materialization to delta-updates-style writes.
		identityTransform := func(s string) string { return s }
