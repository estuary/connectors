...
type queryParams struct {
	...
	MetaOpColumn        *sql.Column
	MetaOpColumnIndex   int
}

func renderQueryTemplate(table sql.Table, tpl *template.Template, bounds []sql.MergeBound, objAndArrayAsJson bool) (string, error) {
	// Find the _meta/op column and its index
	var metaOpColumn *sql.Column
	var metaOpColumnIndex int = -1
	...
	if err := tpl.Execute(&w, &queryParams{
		...
		MetaOpColumn:      metaOpColumn,
		MetaOpColumnIndex: metaOpColumnIndex,
	}); err != nil {
		return "", err
	}
