...
{{ if $.MetaUUIDColumn }}{{ template "uncast" (ColumnWithAlias $.MetaUUIDColumn "l") }}{{ else }}CAST(NULL AS STRING){{ end }} as flow_uuid,
...
{{- end }}
{{- else -}}
(SELECT -1, CAST(NULL AS {{ $.ObjectType }}), CAST(NULL AS STRING), CAST(NULL AS INT64) LIMIT 0) as nodoc
{{- end }}
{{ end }}
