...
def _bindings_use_multi_store_keys(bindings: list[request.ValidateBinding] | list[CaptureBinding]) -> bool:
...
def _validate_multi_store_transition(
    ...
    # Only skip if ALL currently enabled bindings that existed before already use
    # composite keys. A single new binding with composite keys (from discover)
    # should not bypass the transition check for legacy bindings.
    current_binding_names = {b.resourceConfig.name for b in validate_req.bindings}
    relevant_last_bindings = [
        b for b in validate_req.lastCapture.bindings
        if b.resourceConfig.name in current_binding_names
    ]
    if relevant_last_bindings and all(
        "/_meta/store" in b.collection.key for b in relevant_last_bindings
    ):
        return
...
def _validate_key_structure_mismatch(
    ...
    # Single-store captures can safely have mixed key formats — both ["/id"] and
    # ["/_meta/store", "/id"] are valid. Skip mismatch validation to avoid forcing
    # unnecessary backfill when a user discovers new resources on a legacy capture.
    if num_stores <= 1:
        return
...
class Connector(
    ...
    async def discover(
        ...
        # Always use composite keys ["/_meta/store", "/id"] for forward compatibility —
        # avoids requiring backfill if stores are added later.
    ...
    async def validate(
        ...
        _validate_key_structure_mismatch(validate, use_store_in_key, len(validate.config.stores))
