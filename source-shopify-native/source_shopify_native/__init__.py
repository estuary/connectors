...
def _should_use_store_in_key(
    ...
    """Determine whether to include /_meta/store in collection keys.
    ...
    - Single store capture (new or legacy)

    Note: This only affects collection key structure, not state format.
    State is always dictionary-based regardless of key structure.
    """
...
class Connector(
    ...
    async def discover(
        ...
        # New captures: use store in key only if multiple stores
        use_store_in_key = len(discover.config.stores) > 1
        resources = await all_resources(log, self, discover.config, use_store_in_key=use_store_in_key)
