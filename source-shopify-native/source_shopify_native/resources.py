...
from estuary_cdk.http import HTTPError, HTTPMixin, TokenSource, RateLimiter
...
class StoreInitError(Exception):
    ...
    def __init__(self, failed_stores: dict[str, Exception], initialized_stores: list[str]):
...
class StoreHTTP(HTTPMixin):
    """Per-store HTTP wrapper that delegates to a shared session with per-store auth and rate limiting.

    Shares the parent's aiohttp.ClientSession for connection pooling,
    but maintains independent token_source and rate_limiter per store.
    """
    ...
    def __getattr__(self, name: str):
        """Fall back to parent for any attributes not explicitly set."""
    ...
    for model in INCREMENTAL_RESOURCES:
        ...
        def create_open_fn(
            ...
            async def open(
                ...
                state: ResourceState,
                task: Task,
                _all_bindings: list[CaptureBinding] | None = None,
            ):
