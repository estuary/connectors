...
async def process_result(log: Logger, lines: AsyncGenerator[bytes, None]) -> AsyncGenerator[Any, None]:
    ...
    async for line in lines:
        ...
        if "gid://shopify/Product/" in id:
            ...
            if not current_product:
                ...
                log.error("Variant's parent id does not match the current product's id. Check if the JSONL response from Shopify is not ordered correctly.", {
                    "variant.id": id,
            ...
            if not current_variant:
                ...
                log.error("Inventory level's parent id does not match the current variant's id. Check if the JSONL response from Shopify is not ordered correctly.", {
            ...
            if not current_product:
                ...
                log.error("Media's parent id does not match the current product's id. Check if the JSONL response from Shopify is not ordered correctly.", {
                    "media.id": id,
        ...
        elif not id and "gid://shopify/Publication/" in record.get('publication', {}).get('id', ''):
            if not current_product:
                log.error("Found a publication before finding a product.")
                raise RuntimeError()
            elif record.get('__parentId', '') != current_product.get('id', ''):
                log.error("Publication's parent id does not match the current product's id. Check if the JSONL response from Shopify is not ordered correctly.", {
