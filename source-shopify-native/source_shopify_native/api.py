...
# To *hopefully* guard against minor potential distributed clock & data availability issues with
# the bulk GraphQL API, don't let the window end date to be more recent than WINDOW_END_LAG minutes
# in the past.
WINDOW_END_LAG = 1
...
async def fetch_products(
    ...
    end = min(max_end, datetime.now(tz=UTC) - timedelta(minutes=WINDOW_END_LAG))
    ...
    async for record in products.process_result(log, lines):
        ...
        if product.updatedAt > last_seen_dt:
            # If we've yielded any documents, checkpoint them.
