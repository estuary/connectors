...
from source_gong.resources import (
    all_resources,
...
class Connector(
    ...
    async def spec(self, log: Logger, _: request.Spec) -> ConnectorSpec:
        return ConnectorSpec(
            ...
            documentationUrl="https://go.estuary.dev/source-gong",
    ...
    async def discover(
        ...
        resources = await all_resources(log, self, discover.config, rate_limiter)
    ...
    async def validate(
        ...
        rate_limiter = GongRateLimiter()
        await validate_credentials(log, self, validate.config, rate_limiter)

        resources = await all_resources(log, self, validate.config, rate_limiter)
    ...
    async def open(
        ...
        rate_limiter = GongRateLimiter()
        resources = await all_resources(log, self, open.capture.config, rate_limiter)
