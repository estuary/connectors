...
- **Release requirements** — every new connector ships with all three; `source-zuora`'s introduction is the reference shape for each:
  - **CI registration**: add the connector to `.github/workflows/python.yaml` in all three places — the `push` paths list, the `pull_request` paths list, and the `py_connector` matrix (`name`, `type: capture`, `version` matching the connector's `VERSION` file, `usage_rate: "1.0"`).
