...
func (d *Driver) Validate(ctx context.Context, req *pm.Request_Validate) (*pm.Response_Validated, error) {
	...
	for idx, b := range req.Bindings {
		...
		log.WithFields(log.Fields{"groupBy": b.GroupBy, "path": path}).Info("binding")
		...
		for _, field := range b.fields {
			if _, ok := schema.GetProperty(b.object, field.Property.Name); !ok {
				if !schema.HasPropertyGroup(b.object, flowPropertyGroupName) {
					...
					schema.CRM.Objects[b.object].PropertyGroups[flowPropertyGroupName] = flowPropertyGroup
				}
				...
				if err != nil {
					return nil, fmt.Errorf("unable to create property for field %q: %v", field.Property.Name, err)
				}
			}
		}
	}
...
func (m *materialization) NewTransactor(
	...
	for _, mapped := range mappedBindings {
		idProperty, ok := schema.GetProperty(mapped.object, mapped.Key().Name)
		if !ok {
			return nil, fmt.Errorf("property not found: %q", mapped.Key().Name)
		}
