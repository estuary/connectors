...
func (db *sqlserverDatabase) ScanTableChunk(ctx context.Context, info *sqlcapture.DiscoveryInfo, state *sqlcapture.TableState, callback func(event sqlcapture.ChangeEvent) error) (bool, []byte, error) {
	...
	var deleteNullability = make([]bool, len(cnames))           // Whether nil values should be included in delete events (not used for backfill, but required for struct)
	for idx, name := range cnames {
		...
		deleteNullability[idx] = true // Backfill only produces InsertOp, so this isn't actually used
