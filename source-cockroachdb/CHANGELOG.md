...
### Added
- Initial release of the CockroachDB capture connector. Real-time CDC using native sinkless
  ("core") changefeeds streamed over the SQL connection. Supports:
  - Discovery of tables, columns, and (single or composite) primary keys, including tables keyed
    on the synthesized `rowid` and hash-sharded primary keys.
