...
async def _is_side_conversations_enabled(
    ...
    try:
        ...
        side_conv = settings.get("settings", {}).get("side_conversations", {})
        return bool(side_conv.get("show_in_context_panel", False))
    except HTTPError:
        raise
    except Exception as err:
        log.warning(
            f"Could not determine if side conversations are enabled; defaulting to disabled. Error: {err}"
        )
        return False
