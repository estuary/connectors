...
type emrConfig struct {
	...
	SystemsManagerPrefix string `json:"systems_manager_prefix,omitempty" jsonschema:"title=System Manager Prefix,description=Prefix for parameters in Systems Manager as an absolute directory path (must start and end with /). This is required when using Client Credentials for catalog authentication.,default=/estuary/" jsonschema_extras:"pattern=^/.+/$,order=8"`
}
