...
func (t *transactor) loadWorker(ctx context.Context, loaded func(binding int, doc json.RawMessage) error, fileKeys <-chan string) error {
	for fileKey := range fileKeys {
		...
		//
		// 1) An integer for the binding number, surrounded by a single
		// character "quote" which is somewhat arbitrarily chosen as '|'
		// 2) A comma
		// 3) Some number of bytes which are assumed to be a valid JSON object,
		// which is the loaded document, also surrounded by the quote character
		//
		// This format is generated by the "load" EMR job per its write CSV
		// settings.
		...
		for scanner.Scan() {
			...
			} else if binding, err := strconv.Atoi(string(line[1 : commaIdx-1])); err != nil {
