...
class UserPass(BaseModel):
    ...
    # As opposed to the OAuth flow that automatically populates credentials for
    # users, UserPass credentials are populated by the user. Most users won't
    # have their instance_url handy, but keeping the instance_url in the credentials
    # makes downstream handling easier. The connector can determine the instance_url
    # from the access token response, so this is set after the SalesforceTokenSource
    # is initialized.
    ...
    def initialize_instance_url(self, is_sandbox: bool):
        _, instance_url = self.fetch_access_token_and_instance_url(is_sandbox=is_sandbox)
...
class SalesforceTokenSource(TokenSource):
    ...
    def __post_init__(self):
        if isinstance(self.credentials, UserPass):
            # The instance_url does not get set on the credentials with
            # UserPass authentication, so we initialize it here.
            self.credentials.initialize_instance_url(self.is_sandbox)
