...
def try_parse_facebook_api_error(error: HTTPError) -> ParsedFacebookError | None:
    """
    Try to parse an HTTPError into a Facebook exception.

    Returns:
        - DataLimitExceededError if the error indicates data limits were exceeded
        - FacebookAPIError for other Facebook errors
        - None if the response cannot be parsed as a Facebook error
    """
    try:
