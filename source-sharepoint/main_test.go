...
func TestConfigValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "valid config with site_url only",
			cfg: config{
		...
		{
			name: "valid config with site_id only",
			cfg: config{
				SiteID:      "site-123",
				Credentials: validCreds,
			},
		...
		{
			name: "valid config with site_id and path",
			cfg: config{
				SiteID:      "site-123",
				Path:        "/my-folder",
				Credentials: validCreds,
		...
		{
			name: "valid config with site_id and drive_id",
			cfg: config{
				SiteID:      "site-123",
				DriveID:     "drive-456",
				Credentials: validCreds,
		...
		{
			name: "invalid config - both site_url and site_id",
			cfg: config{
			...
			wantErr:     true,
			errContains: "cannot provide both 'site_id' and 'site_url'",
		},
		{
			name: "invalid config - both site_url and path",
			cfg: config{
				SiteURL:     "https://contoso.sharepoint.com/sites/Marketing/Documents",
				Path:        "/my-folder",
				Credentials: validCreds,
			...
			wantErr:     true,
			errContains: "cannot provide both 'site_url' and 'path'",
		},
		{
			name: "invalid config - neither site_url nor site_id",
			cfg: config{
				Credentials: validCreds,
			},
			wantErr:     true,
			errContains: "either 'site_id' or 'site_url' must be provided",
		},
	}
