...
func (c *client) InfoSchema(ctx context.Context, resourcePaths [][]string) (*boilerplate.InfoSchema, error) {
	...
	for rows.Next() {
		...
		is.PushField(boilerplate.EndpointField{
			...
			Type:               c.DataType,
			CharacterMaxLength: 0, // Galaxy does not have max length in information schema
...
// InstallFence is a no-op since materialize-starburst-galaxy doesn't use fencing.
