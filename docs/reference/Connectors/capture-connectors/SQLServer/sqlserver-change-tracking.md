...
| `/discoveryFilters/discover_tables_without_ct` | Discover Tables Without Change Tracking | When set, the connector discovers all tables even if they do not have Change Tracking enabled. Combined as a union with the equivalent setting under Advanced Options. | boolean |  |
