...
Cloud SQL issues server certificates from a per-instance CA by default. Download it from the instance's **Connections** > **Security** page in the Cloud Console, paste it into `ssl_server_ca`, and use `verify_ca`, because per-instance certificates don't name the instance's IP address. Instances configured with the Google-managed shared CA (`GOOGLE_MANAGED_CAS_CA`) are trusted by default, and work with `verify_identity` when you connect to the instance's DNS name.
