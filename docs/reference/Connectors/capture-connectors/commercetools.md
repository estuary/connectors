...
* A commercetools [API Client](https://docs.commercetools.com/api/authorization) with the `view_orders`, `view_customers`, and `view_payments` [scopes](https://docs.commercetools.com/api/scopes), created in the Merchant Center under **Settings > Developer settings > Create new API client**. Note the client ID and secret when the client is created.
