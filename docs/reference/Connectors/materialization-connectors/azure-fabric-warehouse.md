...
| **`/schema`**             | Schema                         | Schema for bound collection tables (unless overridden within the binding resource configuration) as well as the [`flow_checkpoints_v1` metadata table](/concepts/materialization/#the-checkpoints-metadata-table). | string | Required         |
