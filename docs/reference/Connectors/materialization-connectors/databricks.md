...
| `/advanced/disable_staged_file_compression` | Disable Staged File Compression | Files staged to the Unity Catalog volume are compressed with gzip by default. When enabled, they are written uncompressed instead, which uses less CPU at the cost of uploading more data to Databricks. | boolean | `false` |
