---
description: Write delta updates of Estuary collections into an S3 bucket as compressed JSON files. Configure the upload interval, maximum file size, and file path.
---

# JSON Files in Amazon S3

This connector materializes [delta updates](/concepts/materialization/#delta-updates) of
Estuary collections into files in an S3 bucket as newline-delimited JSON: each line of a file is a
single JSON object holding one document. The files are compressed using Gzip compression and
written to S3 as `.json.gz` files.

The delta updates are batched within Estuary, converted to JSON files, and then pushed to the S3
bucket at a time interval that you set. Files are limited to a configurable maximum size. Each
materialized Estuary collection will produce many separate files.

Object and array fields are written as nested JSON values rather than as encoded strings, so
documents keep the shape they have in the collection.
...
| `/jsonConfig/skipNulls`              | Skip Nulls            | Do not write fields with null values to files.                                                                                                | boolean |                  |
...
materializations:
  ${PREFIX}/${mat_name}:
    endpoint:
      connector:
        image: "ghcr.io/estuary/materialize-s3-json:v1"
        config:
          bucket: bucket
...
```
bucket/prefix/path/v0000000000/00000000000000000000.json.gz
bucket/prefix/path/v0000000000/00000000000000000001.json.gz
bucket/prefix/path/v0000000000/00000000000000000002.json.gz
```
