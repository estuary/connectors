...
Airbyte-to-Flow: an Airbyte connector wrapped with `airbyte-to-flow`, a Rust
component that translates between the Airbyte protocol and the Flow capture
protocol.
ATF connectors live in Estuary's [fork of the airbyte repo][estuary-airbyte].
...
5. `BACKFILL` as a shard status, e.g. in `flowctl` shard listings.
   It means a shard replica is replaying its recovery log after starting up
   before it can become `STANDBY` or `PRIMARY`.
...
1. `estuary-cdk`, the Python library for writing capture connectors in this
   repository.
2. The Airbyte CDK, a third-party framework that ATF and imported
...
1. A connector feature flag: one of a comma-separated list at
   `/advanced/feature_flags` in an endpoint config, used to change a
   connector's behavior for one task without changing it for all.
   See [feature_flags.md](feature_flags.md).
2. A runtime feature flag: an entry under `shards.flags` in a task's
   specification, which the runtime reads.
...
2. `flowctl-go`, a separate Go binary.
   Run as `flowctl-go serve consumer`, it is the reactor inside a data plane.
   Run by an operator, it carries the `journals` and `shards` commands
   inherited from `gazctl`, which cover some shard-level operations that
   `flowctl` itself does not.
...
An Airbyte connector wrapped with `estuary-cdk`'s `CaptureShim`, which
adapts it to the Flow capture protocol. These connectors live in this repository.
...
A capture connector written directly with the `estuary-cdk` rather than by
wrapping an third-party connector.
When a native connector replaces an existing ATF or imported connector, its
name takes a `-native` suffix, e.g. `source-salesforce-native`.
The suffix only signals that a name collision had to be resolved, so a
connector without it may also be native.
All new Python connectors are native.
...
The collections and tasks that carry Flow's own operational data.
Each data plane has a raw logs collection and a raw stats collection under
`ops/tasks/` where the runtime collects task logs, including everything a
connector writes to stderr, and task statistics.
Derivations roll them up in two levels: L1 derivations inside each data
plane under `ops/rollups/L1/`, and L2 derivations that merge every data
plane's L1 output into one global product.
Stats materializations then materialize the L2 rollups for reporting and
into the control plane's tables.
See the [`ops-catalog/README.md`][ops-catalog-readme] for additional
details.
...
It is `flowctl-go serve consumer`: it takes shard assignments, starts
connector containers, drives them over the connector protocols, and writes
their output to Gazette journals.
...
3. A derivation transaction: the runtime reads shuffled documents from the
   source collections into the derivation connector, which publishes derived
   documents, and the transaction ends with `Request.Flush` and
   `Request.StartCommit` just as a materialization's does.
4. A transaction in the source or destination database.
...
[ops-catalog-readme]: https://github.com/estuary/flow/blob/master/ops-catalog/README.md
[estuary-airbyte]: https://github.com/estuary/airbyte
