...
class TestSourceFacebookMarketing:
    def test_check_connection_ok(self, api, config, logger_mock):
        ok, error_msg = SourceFacebookMarketing().check_connection(
            logger_mock, config=config
        )
        ...
        logger_mock.info.assert_called_once_with(
            f"Select account {api.return_value.account}"
        )
    ...
    def test_check_connection_future_date_range(self, api, config, logger_mock):
        ...
        assert SourceFacebookMarketing().check_connection(
            logger_mock, config=config
        ) == (
            False,
    ...
    def test_get_custom_insights_action_breakdowns_allow_empty(self, api, config):
        ...
        streams = SourceFacebookMarketing().get_custom_insights_streams(
...
def test_check_config(config_gen, requests_mock):
    ...
    assert command_check(source, config_gen()) == AirbyteConnectionStatus(
    ...
    assert command_check(source, config_gen(end_date=...)) == AirbyteConnectionStatus(
        status=Status.SUCCEEDED, message=None
    )
    assert command_check(source, config_gen(end_date="")) == AirbyteConnectionStatus(
        status=Status.SUCCEEDED, message=None
