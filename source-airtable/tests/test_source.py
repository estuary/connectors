...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            (
                False,
                "401 Client Error: None for url: https://api.airtable.com/v0/meta/bases",
            ),
        ),
    ],
    ids=["success", "fail"],
...
def test_check_connection(
    ...
    requests_mock.get(
        ...
        status_code=status,
        json=fake_bases_response,
    )
    fake_base_id = fake_bases_response.get("bases")[0].get("id")
    # fake the tables based on faked bases
    requests_mock.get(
        f"https://api.airtable.com/v0/meta/bases/{fake_base_id}/tables",
        status_code=status,
        json=fake_tables_response,
    )
...
def test_discover(
    ...
    fake_bases_response,
    fake_tables_response,
    expected_discovery_stream_name,
    requests_mock,
):
    source = SourceAirtable()
    # fake the bases
    ...
    assert [
        stream.name for stream in airbyte_catalog.streams
    ] == expected_discovery_stream_name
...
def test_remove_missed_streams_from_catalog(
    mocker, config, fake_catalog, fake_streams, caplog
):
    logger = logging.getLogger(__name__)
    source = SourceAirtable()
    mocker.patch(
        "source_airtable.source.SourceAirtable.streams", return_value=fake_streams
    )
    streams_before = len(fake_catalog.streams)
    catalog = source._remove_missed_streams_from_catalog(
        logger=logger, config=config, catalog=fake_catalog
    )
