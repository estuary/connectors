...
func (s *replicationStream) poll(ctx context.Context, minSCN, maxSCN int64, transactions []string) error {
	var startSCN = minSCN
	logrus.WithFields(logrus.Fields{
		"minSCN":       minSCN,
		"maxSCN":       maxSCN,
	...
	for {
		...
		if len(transactions) == 0 {
			var newPendingTxs map[string]int64
...
func (s *replicationStream) pendingAndRollbackedTransactions(ctx context.Context, startSCN, endSCN int64) (map[string]int64, map[string]struct{}, error) {
	// we use maps for O(1) lookup
	var excludeXIDs = make(map[string]struct{})
	var txs = make(map[string]int64)
