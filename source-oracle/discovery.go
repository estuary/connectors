...
func predictableColumnOrder(colType any) bool {
	// NUMBER columns without a scale-zero constraint are discovered as
	// JsonType="string" + Format="number" and encoded as raw strings,
	// whose bytewise lex order disagrees with Oracle's numeric ORDER BY
	// (ex: "10000" < "9999").
	if t, ok := colType.(oracleColumnType); ok {
		if t.JsonType == "string" && t.Format == "number" {
			return false
		}
	}
	return true
