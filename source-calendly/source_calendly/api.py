...
async def backfill_entity(
    ...
    min_date = datetime.fromisoformat(page) if page is not None else start_date
...
def _event_window_params(
    org_uri: str,
    scheduled_event_lookback_months: int,
    scheduled_event_lookahead_months: int,
    now: datetime,
) -> dict[str, str | int]:
    start_date = now - timedelta(days=scheduled_event_lookback_months * 30)
    end_date = now + timedelta(days=scheduled_event_lookahead_months * 30)
...
async def fetch_scheduled_events(
    ...
    params = _event_window_params(
        org_uri, scheduled_event_lookback_months, scheduled_event_lookahead_months, now
    )
