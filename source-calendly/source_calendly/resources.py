    ...
    def open(
        ...
        open_binding(
            ...
            fetch_page=functools.partial(
                backfill_entity, entity_cls, org_uri, config.start_date, http
            ),
        )
...
def _scheduled_event_resources(
    http: HTTPMixin, config: EndpointConfig, org_uri: str
) -> list[CalendlyResource]:
    def open(
        fetch_fn: ScheduledEventFetchFn,
        binding: CaptureBinding[ResourceConfig],
        binding_index: int,
        state: ResourceState,
        task: Task,
        ...
        open_binding(
            ...
            fetch_changes=functools.partial(
                fetch_fn,
                org_uri,
                config.scheduled_event_lookback_months,
                config.scheduled_event_lookahead_months,
                http,
            ),
        )
    ...
    return [
        Resource(
            ...
            open=functools.partial(open, fetch_scheduled_events),
            initial_state=ResourceState(
        ...
        Resource(
            ...
            key=["/uri"],
            model=EventInvitee,
            open=functools.partial(open, fetch_event_invitees),
            initial_state=ResourceState(
                inc=ResourceState.Incremental(cursor=config.start_date),
            ),
            initial_config=ResourceConfig(
...
async def _filter_available_resources(
    log: Logger,
    http: HTTPMixin,
    org_uri: str,
    resources: list[CalendlyResource],
) -> list[CalendlyResource]:
    if await is_teams_account(org_uri, http, log):
        return resources

    filtered: list[CalendlyResource] = []
    for resource in resources:
        model = resource.model
        assert isinstance(model, type) and issubclass(model, CalendlyEntity)

        if model.teams_only:
            log.info(f"Skipping {resource.name}: requires a teams account")
        else:
...
async def all_resources(
    ...
    resources = [
        ...
        *_scheduled_event_resources(http, config, org_uri),
        *_snapshot_resources(http, org_uri),
    ]

    return await _filter_available_resources(log, http, org_uri, resources)
