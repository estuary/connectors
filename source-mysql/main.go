...
func (db *mysqlDatabase) connect(ctx context.Context) error {
	...
	if err := db.logServerVariables(logrus.DebugLevel); err != nil {
...
func (db *mysqlDatabase) logServerVariables(level logrus.Level) error {
	...
	logrus.WithFields(fields).Log(level, "queried server variables")
	return nil
}
