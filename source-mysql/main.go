...
func (db *mysqlDatabase) connect(ctx context.Context) error {
	...
	// Infer the location in which captured DATETIME values will be interpreted. The system variable
	// 'time_zone' is preferentially used if it is set on the database.
	var tzName string
	...
	// If the location could not be determined from the database fall back to the timezone from the
	// capture configuration if available.
	if db.datetimeLocation == nil && db.config.Timezone != "" {
