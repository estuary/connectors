...
async def export_survey_responses(
    ...
    try:
        ...
        async def unzipped_stream() -> AsyncGenerator[bytes, None]:
            async for chunk in UnzipStream(body(), flatten=True):
                yield chunk  # type: ignore[return-value]

        processed_any = False
        async for response in IncrementalJsonProcessor(
            input=unzipped_stream(),
            prefix="",
        ...
        if not processed_any:
            log.info(f"Survey {survey_id} export was empty (no responses)")
        else:
            log.info(
                f"Completed processing survey {survey_id}: {response_count} responses"
            )
