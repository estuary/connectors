...
func (d *driver) Pull(open *pc.Request_Open, stream *boilerplate.PullOutput) error {
	...
	for !prevState.isChangeStreamBackfillComplete(changeStreamBindings) {
		...
		if streams, err := c.initializeStreams(ctx, changeStreamBindings, serverInfo.supportsPreImages, cfg.MergeUpdates, cfg.Advanced.ExclusiveCollectionFilter, excludeCollections); err != nil {
			return err
	...
	if len(changeStreamBindings) > 0 {
		...
		streams, err := c.initializeStreams(groupCtx, changeStreamBindings, serverInfo.supportsPreImages, cfg.MergeUpdates, cfg.Advanced.ExclusiveCollectionFilter, excludeCollections)
		if err != nil {
			return err
		}
