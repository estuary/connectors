...
type captureState struct {
	...
	ChangeStreamEmptyBeforeBackfill *bool `json:"change_stream_empty_before_backfill,omitempty"`
...
func (s *captureState) wasChangeStreamEmptyBeforeBackfill() bool {
	if s.ChangeStreamEmptyBeforeBackfill != nil {
		return *s.ChangeStreamEmptyBeforeBackfill
	}
	return false
}
...
func checkBackfillState(state *captureState, oldestOplogEntry time.Time) (stateUpdated bool, restartReason string) {
	...
	if backfillStartTime.IsZero() {
		...
		state.ChangeStreamEmptyBeforeBackfill = &isEmpty
		return true, ""
		...
		state.ChangeStreamEmptyBeforeBackfill = &superHighTechFalse
		return true, ""
	} else if !state.wasChangeStreamEmptyBeforeBackfill() && oldestOplogEntry.IsZero() {
