...
func TestUpdateResourceStates(t *testing.T) {
	prevState := captureState{
		Resources: map[boilerplate.StateKey]resourceState{
			"sk1": {Backfill: backfillState{Done: makePtr(true), LastId: &bson.RawValue{Value: []byte("first")}}},
			"sk2": {Backfill: backfillState{Done: makePtr(true), LastId: &bson.RawValue{Value: []byte("second")}}},
			"sk3": {Backfill: backfillState{Done: makePtr(true), LastId: &bson.RawValue{Value: []byte("third")}}},
		},
