...
func TestCheckBackfillState(t *testing.T) {
	...
	require.True(t, *state.ChangeStreamEmptyBeforeBackfill)
	...
	require.True(t, updated)
	require.Equal(t, "", restartReason)
	require.False(t, *state.ChangeStreamEmptyBeforeBackfill)
	...
	state.ChangeStreamEmptyBeforeBackfill = &superHighTechTrue
	updated, restartReason = checkBackfillState(state, oldestOplogEntry)
	require.False(t, updated)
	require.Equal(t, "", restartReason)
	require.True(t, *state.ChangeStreamEmptyBeforeBackfill)
