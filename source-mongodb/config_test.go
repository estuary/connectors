...
func TestUnmarshalConfig(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			raw, err := json.Marshal(map[string]string{"user": "user", "password": "password", "address": tt.address})
			require.NoError(t, err)
	...
	t.Run("unknown field", func(t *testing.T) {
		var cfg config
		require.ErrorContains(t, pf.UnmarshalStrict([]byte(`{"user": "user", "password": "password", "address": "mongodb://my-mongo.test", "bogus": 1}`), &cfg), `unknown field "bogus"`)
	})
}
