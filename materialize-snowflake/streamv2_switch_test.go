...
func TestStreamV2CheckpointDoesNotSurviveAnotherWritePath(t *testing.T) {
	...
	var state, err = json.Marshal(checkpoint{stateKey: &checkpointItem{
		Table:    "TBL",
		StreamV2: map[string]*streamV2Item{"00000000-ffffffff": {Channel: channel, Counter: 3, KeyEnd: math.MaxUint32}},
	}})
...
func TestStreamV2ReturningToTheWritePathSkipsNewDocuments(t *testing.T) {
	...
	require.Equal(t, int64(3), entries[0]["00000000-ffffffff"].Counter)
	before.stop()
	...
	require.Equal(t, int64(3), after.bindings[0].channels[0].skip)
}
...
func TestStreamV2SwitchOffTheWritePathIsRefused(t *testing.T) {
	...
	// subrange — so that a shard reads back only the item it wrote.
	var itemsFor = func(items ...*streamV2Item) map[string]*streamV2Item {
		var bySubrange = make(map[string]*streamV2Item, len(items))
		for _, item := range items {
			bySubrange[streamV2Range{keyBegin: item.KeyBegin, keyEnd: item.KeyEnd}.key()] = item
		}
		return bySubrange
	}
...
func TestStreamV2WritePathSwitch(t *testing.T) {
	...
	t.Run("a binding moved off this write path with rows in flight duplicates them", func(t *testing.T) {
		...
		require.Equal(t, int64(3), replay.bindings[0].channels[0].skip)
