...
func (bw *bdecWriter) close() error {
	...
	// furiously writing bytes of the serialized output to its writer, which is
	// a network pipe to object storage. This kind of spiky network traffic is
	// probably not optimal for throughput, and it may be worth implementing a
	// kind of async closing strategy here and/or in the parquet writer itself
	// so that the completion of Close() can be awaited in the background while
	// the next file is in progress.
