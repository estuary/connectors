...
type config struct {
	...
	TimestampType snowflakeTimestampType           `json:"timestamp_type" jsonschema:"title=Snowflake Timestamp Type,description=Controls how timestamp columns are stored in Snowflake.,enum=TIMESTAMP_NTZ (normalize to UTC),enum=TIMESTAMP_NTZ (discard TZ),enum=TIMESTAMP_LTZ,enum=TIMESTAMP_TZ" jsonschema_extras:"order=8"`
