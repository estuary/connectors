...
type fileColumnProperties struct {
	...
	MaxIntValue       *big.Int        `json:"maxIntValue"`
	MinIntValue       *big.Int        `json:"minIntValue"`
...
func post[T any](ctx context.Context, c *streamClient, path string, body any) (*T, error) {
	...
	req = req.SetResponseBodyUnlimitedReads(true)
	...
	if !got.IsSuccess() {
		return nil, fmt.Errorf("failed to POST %s: %s: %w", got.Request.URL, got.Status(), got.Error().(*errorResponse))
	}
...
func generateBlobMetadata(
	...
	for _, col := range tracked.columns {
		...
		if col.isInt {
			props.MaxIntValue = col.intStats.maxVal.BigInt()
			props.MinIntValue = col.intStats.minVal.BigInt()
		} else if col.isNum {
		...
		if !col.isInt {
			props.MaxIntValue = big.NewInt(0)
			props.MinIntValue = big.NewInt(0)
		}
