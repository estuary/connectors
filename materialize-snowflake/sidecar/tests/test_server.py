...
def test_configure_and_full_flow(harness):
    ...
    res = harness.call("append", channel="ch_0", start_token="base:0", end_token="base:0", rows=[{"ID": 1}, {"ID": 2}])
...
def test_append_spans_a_token_range(harness):
    ...
    res = harness.call("append", channel="ch_0", start_token="1", end_token="3", rows=[{"ID": 1}, {"ID": 2}, {"ID": 3}])
...
def test_unknown_channel(harness):
    assert configure(harness)["ok"]
    res = harness.call("append", channel="nope", start_token="t", end_token="t", rows=[])
...
def test_invalid_rows(harness):
    assert configure(harness)["ok"]
    assert harness.call("open_channel", database="DB", schema="SCH", table="TBL", channel="ch_0")["ok"]
    res = harness.call("append", channel="ch_0", start_token="t", end_token="t", rows=["not-a-dict"])
...
def test_channels_are_independent(harness):
    ...
    assert harness.call("append", channel="ch_1", start_token="a:0", end_token="a:0", rows=[{"ID": 1}])["ok"]
    assert harness.call("append", channel="ch_2", start_token="b:0", end_token="b:0", rows=[{"ID": 2}])["ok"]
...
def test_receiver_saturated_retries(harness, monkeypatch):
    ...
    res = harness.call("append", channel="ch_0", start_token="t:0", end_token="t:0", rows=[{"ID": 1}])
    ...
    remaining["n"] = 10_000
    res = harness.call("append", channel="ch_0", start_token="t:1", end_token="t:1", rows=[{"ID": 2}])
