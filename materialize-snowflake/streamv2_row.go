...
// encoder, which covers whatever a future converter introduces.
//
// []byte and json.RawMessage are both byte slices but cannot share a case,
// because a BINARY column's bytes have to be base64'd where a VARIANT column's
// are already JSON and are copied through. A type switch separates them by
// exact dynamic type, so each gets the treatment its column needs.
...
// if it carries a line break. A row is written into a shared payload buffer and
// travels to the sidecar as bytes, so its freedom from line breaks should be
// this writer's own property rather than an undocumented guarantee of whoever
// encoded the value. The scan costs nothing measurable next to writing the
// bytes, where compacting every value unconditionally costs more than the whole
// writer.
...
// HTML embedding are written literally, so the bytes differ from the standard
// encoder's while the decoded string is the same.
