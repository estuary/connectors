...
// SDK without parsing them. Everything else, in both directions, is a JSON line.
...
func (c *sidecarClient) call(ctx context.Context, op string, params any, timeout time.Duration, result any) error {
	return c.callWithPayload(ctx, op, params, nil, timeout, result)
}

// callWithPayload issues one request and awaits its response. A non-nil payload
// is written immediately after the request line, which is how an append carries
// its rows; the header must declare the payload's length for the sidecar to know
// where it ends.
func (c *sidecarClient) callWithPayload(ctx context.Context, op string, params any, payload []byte, timeout time.Duration, result any) error {
	...
	var err = c.write(rpcRequest{ID: id, Op: op, Params: params}, payload)
	if err != nil {
...
// keyed by column name, framed as the write path wrote them; rowCount is how many
// it holds.
//
// The rows travel as an opaque payload rather than inside the request, so that
// the sidecar can pass their bytes to the SDK — which wants exactly these bytes
// — without parsing them into Python objects first. That saves the sidecar's
// single reader thread the cost of every row of every append, which is where
// nested documents were being decoded and re-encoded for nothing.
//
// One scan of the payload recovers the extent of every row, which is all the
// sidecar needs; the row count travels in the header so that a payload holding a
// different number of rows than the batch it claims to be is refused rather than
// committed under this batch's offset token.
