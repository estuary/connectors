...
func TestValidateAndApply(t *testing.T) {
	...
	boilerplate.RunValidateAndApplyTestCases(
		...
		func(t *testing.T) {
			...
			_, _ = db.ExecContext(ctx, fmt.Sprintf("drop table %s;", testDialect.Identifier(cfg.Database, resourceConfig.Schema, resourceConfig.Table)))
		},
	)
}
...
func TestValidateAndApplyMigrations(t *testing.T) {
	...
	sql.RunValidateAndApplyMigrationsTests(
		...
		func(t *testing.T, cols []string, values []string) {
			...
			keys = append(keys, testDialect.Identifier("second_root"))
			values = append(values, "'{}'")
			q := fmt.Sprintf("insert into %s (%s) VALUES (%s);", testDialect.Identifier(cfg.Database, resourceConfig.Schema, resourceConfig.Table), strings.Join(keys, ","), strings.Join(values, ","))
		...
		func(t *testing.T) string {
			...
			rows, err := sql.DumpTestTable(t, db, testDialect.Identifier(cfg.Database, resourceConfig.Schema, resourceConfig.Table))
