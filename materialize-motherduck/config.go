...
type stagingBucketGCSConfig struct {
	...
	BucketBucketPathGCS string `json:"bucketPathGCS,omitempty" jsonschema:"title=Bucket Path,description=An optional prefix that will be used to store objects in the staging bucket." jsonschema_extras:"order=4"`
}
...
func (c *config) toBucketAndPath(ctx context.Context) (blob.Bucket, string, error) {
	...
	case stagingBucketTypeGCS:
		...
		path = c.StagingBucket.BucketBucketPathGCS
	}
