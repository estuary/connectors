...
def _make_request(
    ...
    return make_mocked_request("POST", path, headers=headers or {})
...
class TestUrlMatch:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("*", "all"),
            ("/webhooks/events", "webhooks_events"),
            ("/v2/events", "v2_events"),
            ("/foo/{bar}/baz", "foo_bar_baz"),
        ],
    )
    def test_display_name(self, value: str, expected: str):
        assert UrlMatch(value=value).display_name == expected
