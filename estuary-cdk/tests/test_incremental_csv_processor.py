...
"Bob Johnson","Simple product",10.00'''
    ...
    def csv_with_special_chars(self, delimiter: str = '|') -> str:
        ...
        return f"""name{delimiter}description{delimiter}notes
...
Bob{delimiter}"Quoted product"{delimiter}Notes with {delimiter} delimiter"""
    ...
    async def test_quoted_fields(self, csv_with_quotes):
        ...
        assert rows[0]["description"] == 'A "great" product'
        assert "newline" in rows[1]["description"]  # Multi-line field
    ...
    async def test_different_delimiters(self):
        ...
        for delimiter in delimiters:
            ...
            processor = IncrementalCSVProcessor(chunk_iterator, config)
            ...
            assert rows[0]["description"] == "Product with, comma", f"Failed with delimiter: {repr(delimiter)}"
    ...
    async def test_single_row(self):
        ...
        assert len(rows) == 1
        assert rows[0] == {"name": "John", "age": "25"}
    ...
    async def test_unicode_decode_error(self):
        ...
        invalid_utf8 = b"name,value\ntest,\xff\xfe"  # Invalid UTF-8 sequence.

        async def invalid_byte_iterator():
            yield invalid_utf8

        processor = IncrementalCSVProcessor(invalid_byte_iterator())
    ...
    async def test_malformed_csv_with_unmatched_quotes(self):
        ...
        processor = IncrementalCSVProcessor(malformed_iterator())
    ...
    async def test_unicode_content(self):
        ...
        unicode_csv = b"""name,city,country
...
\xe5\xbc\xa0\xe4\xb8\x89,\xe5\x8c\x97\xe4\xba\xac,\xe4\xb8\xad\xe5\x9b\xbd"""
        ...
        processor = IncrementalCSVProcessor(unicode_iterator())
    ...
    async def test_large_dataset_processing(self, large_csv_data):
        ...
        assert isinstance(first_row, dict) and isinstance(last_row, dict)
    ...
    async def test_chunk_boundaries_split_records(self):
        ...
        csv_data = "name,description\nJohn,Very long description that will definitely be split across multiple small chunks\nJane,Short desc"
        ...
        assert len(rows) == 2
        assert "Very long description" in rows[0]["description"]
    ...
    async def test_csv_with_inconsistent_fields_is_allowed(self):
        ...
        processor = IncrementalCSVProcessor(csv_iterator())
        ...
        assert len(rows) == 4
        assert rows[0] == {"name": "John", "age": "25", "city": "New York"}
        assert rows[1] == {"name": "Jane", "age": None, "city": None}
        assert rows[2] == {"name": "Bob", "age": "35", "city": None}
        assert rows[3] == {"name": "", "age": "", "city": ""}  # Empty row with empty strings.
    ...
    async def test_headers_only(self):
        ...
        processor = IncrementalCSVProcessor(header_iterator())
    ...
    async def test_very_long_field(self):
        ...
        assert rows[1]["description"] == "Short"
