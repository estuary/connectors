...
async def test_unzip_stream(zip_content, expected_files_lines):
    ...
    found = {}
    async for filename, file_bytes in UnzipStream(bytes_gen(zip_bytes)):
        lines = []
        buffer = b""
        async for chunk in file_bytes:  # type: ignore[assignment]
            buffer += chunk
        for line in buffer.split(b"\n"):
            if line or buffer.endswith(b"\n"):
                lines.append(line)
        if lines and lines[-1] == b"":
            lines.pop()  # Remove trailing empty line if present
        found[filename.decode() if isinstance(filename, bytes) else filename] = lines
    assert found == expected_files_lines
