...
class _AsyncByteReader(aiocsv.protocols.WithAsyncRead):
    """
    Async byte reader that handles incremental UTF-8 decoding for aiocsv.

    This class wraps an async byte generator and provides incremental decoding
    to handle multi-byte characters that may be split across chunk boundaries.

    Example usage:
    ```python
    async_reader = AsyncByteReader(byte_generator, 'utf-8')
    async for row in aiocsv.AsyncDictReader(async_reader):
        process(row)
    ```
    """
...
class IncrementalCSVProcessor(Generic[T]):
    ...
    When a Pydantic model is provided, rows are validated and returned as model instances.
    When no model is provided, raw dicts are returned for maximum performance.
    ...
    Example usage without validation (returns dicts):
    ```python
    ...
    async for row in IncrementalCSVProcessor(byte_iterator, MyModel, config):
    ...
    async for row in IncrementalCSVProcessor(byte_iterator, MyModel, validation_context=context):
        do_something_with(row)
    ```
    ...
    async for row in IncrementalCSVProcessor(byte_iterator, fieldnames=fieldnames):
        do_something_with(row)
    ```
    """
    ...
    async def __anext__(self) -> T:
        ...
        try:
            ...
            if self.streamed_item_cls is not None:
                return self.streamed_item_cls.model_validate(row_data, context=self.validation_context)  # type: ignore[return-value]
            return row_data  # type: ignore[return-value]
        except StopAsyncIteration:
            raise
