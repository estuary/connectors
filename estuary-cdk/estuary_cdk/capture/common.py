...
for efficient incremental updates (e.g., tracking completed board IDs).
...
async def _binding_backfill_task(
    ...
    def initialize_connector_state(state: ResourceState.Backfill) -> ConnectorState:
    ...
    connector_state = initialize_connector_state(state)
    ...
    while True:
        ...
        async for item in pages:
            if isinstance(item, BaseDocument) or (isinstance(item, dict) and not is_cursor_dict(item)):
                ...
                if isinstance(item, dict) and is_cursor_dict(item):
                    ...
                    if state.next_page is None:
                        ...
                        checkpoint_state = ResourceState.Backfill(cutoff=state.cutoff, next_page=item)
                        state_to_checkpoint = initialize_connector_state(checkpoint_state)
                    else:
                        raise RuntimeError("Implementation error: dictionary PageCursor must be")
                else:
                    state.next_page = item
                    state_to_checkpoint = connector_state

                task.log.info("backfill yielded PageCursor", {"item": item, "state.next_page": state.next_page})
                await asyncio.sleep(2)

                task.checkpoint(state_to_checkpoint)
                task.log.info("checkpointed backfill state", {"state": state_to_checkpoint})
                done = False
                await asyncio.sleep(2)
