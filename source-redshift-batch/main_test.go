...
func TestBasicCapture(t *testing.T) {
	...
	var controlURI = fmt.Sprintf(`postgres://%s:%s@%s/%s?sslmode=require`, *dbControlUser, *dbControlPass, *dbAddress, *dbName)
	...
	require.NoError(t, err)
	t.Cleanup(func() { conn.ExecContext(ctx, fmt.Sprintf("DROP TABLE IF EXISTS %s", tableName)) })
	_, err = conn.ExecContext(ctx, fmt.Sprintf("CREATE TABLE %s(id INTEGER PRIMARY KEY, data TEXT)", tableName))
	require.NoError(t, err)
	...
	go func() {
		for i := 0; i < 250; i++ {
			...
			_, err = conn.ExecContext(ctx, fmt.Sprintf("INSERT INTO %s VALUES ($1, $2)", tableName), i, fmt.Sprintf("Value for row %d", i))
			require.NoError(t, err)
