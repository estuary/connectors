        ...
        while chunks_to_process:
            ...
            try:
                ...
                if len(current_chunk) == 1:
                    log.error(
                        f"Board {current_chunk[0]} cannot be fetched with or without workspace kind field. This may be an issue with the board or Monday's API."
                    )
                    raise
                ...
                log.debug(
                    f"Splitting chunk of {len(current_chunk)} into {len(left_chunk)} and {len(right_chunk)}"
                )
