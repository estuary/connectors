...
async def get_items_from_boards(
    ...
    if not board_ids or len(board_ids) == 0:
        log.error("No board IDs provided to get_items_from_boards.")
        raise ValueError("No board IDs provided to get_items_from_boards.")

    log.debug(f"Fetching items for boards {board_ids}.")

    if items_limit is not None:
        if items_limit <= 0:
            log.error("Invalid items_limit provided, must be greater than 0.")
