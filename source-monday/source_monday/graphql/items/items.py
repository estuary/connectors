...
async def fetch_items(
    ...
    total_batches = (len(board_ids) + batch_size - 1) // batch_size

    for i in range(0, len(board_ids), batch_size):
        batch_ids = board_ids[i : i + batch_size]
        batch_number = i // batch_size + 1
        log.debug(
            "Processing items batch for boards",
            {
                "batch_number": batch_number,
                "total_batches": total_batches,
                "boards_in_batch": len(batch_ids),
                "batch_size": batch_size,
                "is_cache_request": for_cache,
                "date_range": f"{start} to {end}" if start and end else "all time",
            }
        )
...
async def _fetch_batch_items_depth_first(
    ...
    while cursors:
        ...
        log.debug(
            "Processing pagination cursor for items batch",
            {
                "cursor": cursor[:50] + "..." if len(cursor) > 50 else cursor,
                "remaining_cursors": len(cursors),
                "batch_boards": len(batch_ids),
            }
        )
