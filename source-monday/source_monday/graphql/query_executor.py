        ...
        try:
            ...
            if remainder.data and remainder.data.complexity:
                ...
                if remainder.data.complexity.after <= LOW_COMPLEXITY_BUDGET:
                    log.debug(
                        f"Low complexity budget reached. Waiting for reset in {remainder.data.complexity.reset_in_x_seconds} seconds."
                    )
                ...
                if error.extensions:
                    if error.extensions.complexity and error.extensions.maxComplexity:
                        if error.extensions.complexity > error.extensions.maxComplexity:
                            log.warning(
                                "GraphQL query complexity exceeded maximum allowed complexity. Will not retry this query since it will not succeed.",
                                {
                                    "complexity": error.extensions.complexity,
                                    "max_complexity": error.extensions.maxComplexity,
                                    "query": query,
                                    "variables": variables,
                                },
                            )
