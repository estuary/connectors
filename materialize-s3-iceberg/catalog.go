...
func buildRestCatalog(ctx context.Context, cfg *config) (icebergcatalog.Catalog, error) {
	...
	// Bypass catalog credential vending only for custom S3 endpoints (local,
	// R2, MinIO) combined with access-key credentials. Standard AWS users and
	// AWSIAM users rely on the catalog to vend or manage credentials via its
	// normal OAuth/STS flow. Both the structured Credentials field and the
	// legacy aws_access_key_id / aws_secret_access_key fields supply direct
	// S3 credentials that don't need catalog vending.
	isCustomEndpointWithAccessKey := cfg.S3Endpoint != "" &&
		(strVal(cfg.AWSAccessKeyID) != "" ||
			(cfg.Credentials != nil && cfg.Credentials.AuthType == filesink.AWSAccessKey))

	if isCustomEndpointWithAccessKey {
