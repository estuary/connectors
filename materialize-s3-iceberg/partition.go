...
func partitionSpecMismatch(existing, desired *iceberg.PartitionSpec) error {
	...
	desiredFields := make([]iceberg.PartitionField, 0, desired.NumFields())
	for _, f := range desired.Fields() {
		desiredFields = append(desiredFields, f)
	}
	for i, f := range existing.Fields() {
		if !f.Equals(desiredFields[i]) {
			return mismatch()
		}
	}
	return nil
...
func partitionLogicalTypes(cols []partitionColumn) map[int]string {
