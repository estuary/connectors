...
class GCSClient:
    """Asynchronous Google Cloud Storage client using HTTP requests."""
    ...
    async def _list_files(
        ...
        Args:
            max_results: Maximum number of objects to return per page
            page_token: Token for pagination

        Returns:
            GCSListResponse containing items and pagination info
        """
        params: Dict[str, Any] = {
    ...
    async def list_all_files(
        ...
        while True:
            result = await self._list_files(prefix=prefix, globPattern=globPattern, page_token=page_token)
    ...
    async def stream_csv(
        ...
        Args:
            object_name: Name/path of the CSV object in the bucket
            model_class: Pydantic model class to validate CSV rows against

        Yields:
            Validated instances of the model class
        """
