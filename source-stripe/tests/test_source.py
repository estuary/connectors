...
message = "if lookback_window_days is not set should not affect cursor value"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            10,
            now_dt.timestamp(),
            int(now_dt.timestamp() - SECONDS_IN_DAY * 10),
...
def test_lookback_window(
    ...
    assert (
        inv_stream.get_start_timestamp({"created": current_state}) == expected
    ), message


def test_source_streams(request):
    sample_file = request.fspath.dirname + "/../sample_files/config.json"
...
def test_source_check_connection_ok(mocked_client, config, logger_mock):
    success, error = SourceStripe().check_connection(
        logger_mock,
        config=config,
    )
...
def test_source_check_connection_failure(mocked_client, config, logger_mock):
    ...
    assert SourceStripe().check_connection(logger_mock, config=config) == (
        False,
        exception,
    )
