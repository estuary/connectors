    ...
    if (
        ...
        for account_id in account_ids:
            ...
            if not inc_state_exists and not backfill_state_exists:
                task.log.info(
                    f"New account id {account_id} found, will update connector state."
                )
                state.inc[account_id] = initial_state.inc[account_id]
                state.backfill[account_id] = initial_state.backfill[account_id]
                should_checkpoint = True
            elif (
                (not inc_state_exists and backfill_state_exists)
                or (inc_state_exists and not backfill_state_exists)
                and not should_backfill
            ):
                task.log.warning(
                    f"Misalignment in backfill and incremental states for account id {account_id}. This is unexpected behavior and the connector will automatically backfill."
                )
                should_backfill = True

        if should_backfill:
            task.log.info(
                f"Backfilling {binding.stateKey} due to missaligned backfill and incremental states."
            )
            task.stopping.event.set()
            task.checkpoint(ConnectorState(backfillRequests={binding.stateKey: True}))
