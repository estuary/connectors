...
def check_accessibility(
    ...
    The Stripe API response has a 200 or 404 status code if the stream is accessible.

    404 status codes are received when checking a child stream that can be inaccessible when its parent stream is accessible.
    """
    code = requests.get(url=url, auth=HTTPBasicAuth(access_token, "")).status_code
...
async def all_resources(
    ...
    if is_restricted_api_key:
        ...
        if not check_accessibility(config.credentials.access_token, events_url):
    ...
    for element in STREAMS:
        ...
        if is_restricted_api_key:
            ...
            is_accessible_stream = check_accessibility(config.credentials.access_token, url)
        ...
        if is_accessible_stream:
            ...
            for child in children:
                ...
                if child.get("discoverPath", None):
                    ...
                    is_accessible_child = check_accessibility(config.credentials.access_token, url)
    ...
    for stream in REGIONAL_STREAMS:
        ...
        if check_accessibility(config.credentials.access_token, url):
