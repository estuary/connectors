...
// TestLookupTranEndTimesPageIntegration drives a known sequence of
// transactions against a real SQL Server (one multi-row, two singletons),
...
func TestLookupTranEndTimesPageIntegration(t *testing.T) {
	...
	var tableName = fmt.Sprintf("dbo.tsms_probe_%s", probeID)
	var captureInstance = fmt.Sprintf("dbo_tsms_probe_%s", probeID)
	...
	runSQL(fmt.Sprintf(`IF EXISTS (SELECT 1 FROM cdc.change_tables WHERE capture_instance = N'%s') EXEC sys.sp_cdc_disable_table @source_schema = 'dbo', @source_name = 'tsms_probe_%s', @capture_instance = N'%s'`, captureInstance, probeID, captureInstance))
	runSQL(fmt.Sprintf(`IF OBJECT_ID(N'%s', 'U') IS NOT NULL DROP TABLE %s`, tableName, tableName))
	runSQL(fmt.Sprintf(`CREATE TABLE %s (id INT PRIMARY KEY, val NVARCHAR(64))`, tableName))
	runSQL(fmt.Sprintf(`EXEC sys.sp_cdc_enable_table @source_schema = 'dbo', @source_name = 'tsms_probe_%s', @role_name = NULL, @capture_instance = N'%s'`, probeID, captureInstance))
	t.Cleanup(func() {
		_, _ = conn.ExecContext(ctx, fmt.Sprintf(`IF EXISTS (SELECT 1 FROM cdc.change_tables WHERE capture_instance = N'%s') EXEC sys.sp_cdc_disable_table @source_schema = 'dbo', @source_name = 'tsms_probe_%s', @capture_instance = N'%s'`, captureInstance, probeID, captureInstance))
		_, _ = conn.ExecContext(ctx, fmt.Sprintf(`IF OBJECT_ID(N'%s', 'U') IS NOT NULL DROP TABLE %s`, tableName, tableName))
	})

	// Capture the LSN range bounds that bracket our test workload.
	var fromLSN LSN
	require.NoError(t, conn.QueryRowContext(ctx, "SELECT sys.fn_cdc_get_max_lsn()").Scan(&fromLSN))

	runSQL(fmt.Sprintf(`BEGIN TRANSACTION; INSERT INTO %s VALUES (1, 'a'), (2, 'b'); COMMIT`, tableName))
	time.Sleep(250 * time.Millisecond)
	runSQL(fmt.Sprintf(`INSERT INTO %s VALUES (3, 'c')`, tableName))
	time.Sleep(250 * time.Millisecond)
	runSQL(fmt.Sprintf(`INSERT INTO %s VALUES (4, 'd')`, tableName))

	if _, err := conn.ExecContext(ctx, "EXEC sys.sp_cdc_scan"); err != nil {
		t.Logf("sys.sp_cdc_scan returned: %v (continuing - this is non-fatal if a scan is already running)", err)
	}

	var toLSN LSN
	require.NoError(t, conn.QueryRowContext(ctx, "SELECT sys.fn_cdc_get_max_lsn()").Scan(&toLSN))
	require.NotEmpty(t, toLSN)
	require.Equalf(t, 1, bytes.Compare(toLSN, fromLSN), "toLSN must advance past fromLSN; fromLSN=%X toLSN=%X", fromLSN, toLSN)

	// Read the LSN of every change row (so we can cross-check the lookup).
	type changeRow struct {
		ID  int
		LSN []byte
	}
	rows, err := conn.QueryContext(ctx, fmt.Sprintf(`SELECT id, __$start_lsn FROM cdc.%s_CT ORDER BY __$start_lsn, __$seqval`, captureInstance))
	require.NoError(t, err)
	var changes []changeRow
	for rows.Next() {
		var c changeRow
		require.NoError(t, rows.Scan(&c.ID, &c.LSN))
		changes = append(changes, c)
	}
	require.NoError(t, rows.Err())
	require.NoError(t, rows.Close())
	require.Len(t, changes, 4, "expected change-table rows for ids 1..4 (was sp_cdc_scan effective?)")

	require.Equal(t, changes[0].LSN, changes[1].LSN, "rows from one transaction must share start_lsn")
	require.NotEqual(t, changes[1].LSN, changes[2].LSN, "row 3 must have a different LSN than rows 1/2")
	require.NotEqual(t, changes[2].LSN, changes[3].LSN, "row 4 must have a different LSN than row 3")

	var rs = &sqlserverReplicationStream{conn: conn}
	tranEndTimes, pageEnd, err := rs.lookupTranEndTimesPage(ctx, fromLSN, toLSN, tranEndTimesPageSize)
	require.NoError(t, err)
	require.NotEmpty(t, tranEndTimes, "lookup should return entries for our 3 transactions")
	require.True(t, bytes.Equal(pageEnd, toLSN), "single page should cover the entire requested range; got pageEnd=%X toLSN=%X", pageEnd, toLSN)

	tsMsByID := make(map[int]int64, 4)
	for _, c := range changes {
		ts, ok := tranEndTimes[string(c.LSN)]
		require.Truef(t, ok, "missing ts_ms for row id=%d lsn=%X", c.ID, c.LSN)
		tsMsByID[c.ID] = ts.UnixMilli()
	}

	require.Equal(t, tsMsByID[1], tsMsByID[2], "ids 1 and 2 share a transaction and must share ts_ms")
	require.NotZero(t, tsMsByID[1], "ts_ms must be populated with a real commit time")

	require.LessOrEqual(t, tsMsByID[1], tsMsByID[3], "id 3 commits after id 1/2; ts_ms must be non-decreasing")
	require.LessOrEqual(t, tsMsByID[3], tsMsByID[4], "id 4 commits after id 3; ts_ms must be non-decreasing")

	for _, c := range changes {
		var direct sql.NullTime
		require.NoError(t, conn.QueryRowContext(ctx, "SELECT tran_end_time FROM cdc.lsn_time_mapping WHERE start_lsn = @p1", []byte(c.LSN)).Scan(&direct))
		require.True(t, direct.Valid)
		ts := tranEndTimes[string(c.LSN)]
		require.Truef(t, ts.Equal(direct.Time), "lookup-returned tran_end_time for LSN %X (%v) must equal value read directly from cdc.lsn_time_mapping (%v)", c.LSN, ts, direct.Time)
	}

	t.Logf("validated 4 rows across 3 transactions: id1/2 share ts_ms=%d, id3 ts_ms=%d, id4 ts_ms=%d",
		tsMsByID[1], tsMsByID[3], tsMsByID[4])
}
