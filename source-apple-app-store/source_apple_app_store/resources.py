...
API_RESOURCES: list[tuple[type[AppleResource], ApiFetchPageFn, ApiFetchChangesFn]] = [
...
def _create_initial_state(app_ids: list[str]) -> ResourceState:
    ...
    initial_state = ResourceState(
        inc={app_id: ResourceState.Incremental(cursor=cutoff) for app_id in app_ids},
        backfill={
    ...
    if (
        ...
        for app_id in app_ids:
            ...
            if not inc_state_exists and not backfill_state_exists:
                task.log.info(f"Initializing new subtask state for app id {app_id}.")
                state.inc[app_id] = deepcopy(initial_state.inc[app_id])
                state.backfill[app_id] = deepcopy(initial_state.backfill[app_id])
                ...
                task.log.info(
                    f"Backfilling subtask for app id {app_id} due to missing incremental state."
                )
                state.inc[app_id] = deepcopy(initial_state.inc[app_id])
                state.backfill[app_id] = deepcopy(initial_state.backfill[app_id])
                should_checkpoint = True
...
async def api_resources(
    ...
    return [
        Resource(
            ...
            key=sorted(model.primary_keys),
            model=model,
            open=functools.partial(open, app_ids, fetch_changes_fn, fetch_page_fn),
            initial_state=initial_state,
            initial_config=ResourceConfig(name=model.name, interval=timedelta(hours=1)),
            schema_inference=True,
        )
...
async def all_resources(
    ...
    log.debug(
        f"Discovering resources for config: {config.model_dump_json(indent=2)}"
    )
