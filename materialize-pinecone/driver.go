...
func (d driver) Validate(ctx context.Context, req *pm.Request_Validate) (*pm.Response_Validated, error) {
	...
	if indexDescribe.Database.PodType == starterPodType {
		...
		if len(req.Bindings) > 1 {
			return nil, fmt.Errorf(
				"cannot materialize more than 1 binding to index '%s' which has pod type '%s'. consider materializing fewer bindings, or using an index with a different pod type",
				cfg.Index,
				starterPodType,
			)
		}
		...
		if len(req.Bindings) == 1 {
			...
			if res.Namespace != "" {
				return nil, fmt.Errorf(
					"namespace must be left blank when materializing to index '%s' with pod type '%s', since these pod types do not support namespaces: namespace was '%s'",
					cfg.Index,
					starterPodType,
					res.Namespace,
				)
