...
def test_get_json_schema():
    query_object = MagicMock(
        return_value={
            "a": Obj(
                data_type=Obj(name="ENUM"), is_repeated=False, enum_values=["a", "aa"]
            ),
            "b": Obj(
                data_type=Obj(name="ENUM"), is_repeated=True, enum_values=["b", "bb"]
            ),
    ...
    instance = CustomQueryMixin(
        config={"query": Obj(fields=["a", "b", "c", "d", "e", "f"])}
    )
