...
from google.ads.googleads.v25.services.services.google_ads_service.pagers import (
    SearchPager,
)
...
def get_date_params(
    ...
    if start_date.add(days=1).date() >= end_date.date():
        return (
            start_date.add(days=1).to_date_string(),
            start_date.add(days=2).to_date_string(),
        )
...
def chunk_date_range(
    ...
    if days_of_data_storage:
        start_date = max(
            start_date,
            pendulum.now().subtract(days=days_of_data_storage - conversion_window),
        )
    ...
    while start_date < end_date:
        logger.debug(
            "In 'while start_date < end_date' loop",
            {
                "start_date": start_date,
                "end_date": end_date,
            },
        )
        start, end = get_date_params(
...
class GoogleAdsStream(Stream, ABC):
    ...
    def get_query(self, stream_slice: Mapping[str, Any]) -> str:
        ...
        query = GoogleAds.convert_schema_into_query(
            schema=self.get_json_schema(), report_name=self.name
        )
        self.logger.debug(
            "Exiting GoogleAdsStream.get_query",
            {
                "query": query,
            },
        )
    ...
    def read_records(
        self, sync_mode, stream_slice: Optional[Mapping[str, Any]] = None, **kwargs
    ) -> Iterable[Mapping[str, Any]]:
        self.logger.debug("Entering GoogleAdsStream.read_records")
        self.base_sieve_logger.bump()
        self.base_sieve_logger.info(
            f"Read records using g-ads client. Stream slice is {stream_slice}"
        )
        ...
        try:
            ...
            response_records = self.google_ads_client.send_request(
                self.get_query(stream_slice), customer_id=customer_id
            )
            ...
            for error in exc.failure.errors:
                if (
                    ...
                    self.logger.error(
                        "Ignoring CUSTOMER_NOT_ENABLED error",
    ...
    def stream_slices(
        ...
        for customer in self.customers:
            ...
            if stream_state.get(customer.id):
                start_date = (
                    stream_state[customer.id].get(self.cursor_field) or self._start_date
                )
            ...
            logger.info(
                f"Generating slices for customer {customer.id}. Start date is {start_date}, end date is {end_date}"
            )
            ...
            try:
                ...
                for record in records:
                    ...
                    if current_state:
                        ...
                        cursor_value = (
                            max(date_in_current_stream, date_in_latest_record)
                        ).to_date_string()
                        ...
                        continue
                    self.state = {
                        customer_id: {self.cursor_field: record[self.cursor_field]}
                    }
                    self.logger.info(
                        f"Initialized state for customer {customer_id}. Full state is {self.state}."
                    )
                    yield record
                    continue
                ...
                if (
                    error.error_code.request_error
                    ...
                    stream_slice["start_date"] = self.current_state(
                        customer_id, default=stream_slice["start_date"]
                    )
                    self.logger.info(
                        f"Retry reading records from where it crushed with a modified slice: {stream_slice}"
                    )
                else:
                ...
                self.logger.info(
                    "Current slice has been read. Exiting IncrementalGoogleAdsStream.read_records()"
                )
                return
...
class Accounts(IncrementalGoogleAdsStream):
    """
    Accounts stream: https://developers.google.com/google-ads/api/fields/v25/customer
    """
...
class Campaigns(IncrementalGoogleAdsStream):
    """
    Campaigns stream: https://developers.google.com/google-ads/api/fields/v25/campaign
    """
...
class CampaignLabels(GoogleAdsStream):
    """
    Campaign labels stream: https://developers.google.com/google-ads/api/fields/v25/campaign_label
    """
...
class AdGroups(IncrementalGoogleAdsStream):
    """
    AdGroups stream: https://developers.google.com/google-ads/api/fields/v25/ad_group
    """
...
class AdGroupLabels(GoogleAdsStream):
    """
    Ad Group Labels stream: https://developers.google.com/google-ads/api/fields/v25/ad_group_label
    """
...
class AdGroupAds(IncrementalGoogleAdsStream):
    """
    AdGroups stream: https://developers.google.com/google-ads/api/fields/v25/ad_group_ad
    """
...
class AdGroupAdLabels(GoogleAdsStream):
    """
    Ad Group Ad Labels stream: https://developers.google.com/google-ads/api/fields/v25/ad_group_ad_label
    """
...
class AccountPerformanceReport(IncrementalGoogleAdsStream):
    """
    AccountPerformanceReport stream: https://developers.google.com/google-ads/api/fields/v25/customer
...
class AdGroupAdReport(IncrementalGoogleAdsStream):
    """
    AdGroupAdReport stream: https://developers.google.com/google-ads/api/fields/v25/ad_group_ad
...
class DisplayKeywordPerformanceReport(IncrementalGoogleAdsStream):
    """
    DisplayKeywordPerformanceReport stream: https://developers.google.com/google-ads/api/fields/v25/display_keyword_view
...
class DisplayTopicsPerformanceReport(IncrementalGoogleAdsStream):
    """
    DisplayTopicsPerformanceReport stream: https://developers.google.com/google-ads/api/fields/v25/topic_view
...
class ShoppingPerformanceReport(IncrementalGoogleAdsStream):
    """
    ShoppingPerformanceReport stream: https://developers.google.com/google-ads/api/fields/v25/shopping_performance_view
...
class UserLocationReport(IncrementalGoogleAdsStream):
    """
    UserLocationReport stream: https://developers.google.com/google-ads/api/fields/v25/user_location_view
...
class GeographicReport(IncrementalGoogleAdsStream):
    """
    UserLocationReport stream: https://developers.google.com/google-ads/api/fields/v25/geographic_view
    """
...
class KeywordReport(IncrementalGoogleAdsStream):
    """
    UserLocationReport stream: https://developers.google.com/google-ads/api/fields/v25/keyword_view
    """
...
class ClickView(IncrementalGoogleAdsStream):
    """
    ClickView stream: https://developers.google.com/google-ads/api/reference/rpc/v25/ClickView
    """
...
class GeoTargetConstant(GoogleAdsStream):
    """
    GeoTargetConstant stream: https://developers.google.com/google-ads/api/fields/v25/geo_target_constant
    """
