...
- Child resources listing content under a parent: `campaign_actions`, `broadcast_actions`, `newsletter_contents` and `transactional_contents`. Each issues one request per parent and polls hourly. The parent is recorded at `/_meta/parent_id` rather than read from the response, because these routes report it inconsistently — newsletter contents carry `newsletter_id`, transactional contents carry no parent reference at all.
...
- Segment membership is not captured. That endpoint pages by offset over a set that changes while it is being read, returns a partial result with a 200 while a segment is rebuilding, and is the only Customer.io resource sized by the profile base rather than by workspace configuration.
