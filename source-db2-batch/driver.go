...
type documentMetadata struct {
	Polled time.Time `json:"polled" jsonschema:"title=Polled Timestamp,description=The time at which the update query which produced this document was executed."`
...
func (drv *BatchSQLDriver) Pull(open *pc.Request_Open, stream *boilerplate.PullOutput) error {
	...
	var capture = &capture{
		...
		Semaphore:       semaphore.NewWeighted(maxConcurrentQueries),
	}
...
func (drv *BatchSQLDriver) buildQuery(res *Resource, state *streamState) (string, error) {
	...
	var templateArg = map[string]any{
		...
		"CursorFields": cursorNames, // Pass unquoted names; template will quote them
...
var queryPlaceholderRegexp = regexp.MustCompile(`([?]|@flow_cursor_value\[[0-9]+\])`)

// expandQueryPlaceholders simulates numbered query placeholders by replacing
// each numbered placeholder `@flow_cursor_value[N]` with a question mark while
// building an output argument list corresponding to the usage sequence. This is
// necessary because DB2 uses positional `?` placeholders and doesn't support
// numbered placeholders natively, but multi-column cursors require the same
// value to appear multiple times in the WHERE clause.
func expandQueryPlaceholders(query string, argvals []any) (string, []any, error) {
	...
	query = queryPlaceholderRegexp.ReplaceAllStringFunc(query, func(param string) string {
		if param == "?" {
			prevIndex++
			if prevIndex < int64(len(argvals)) {
		...
		if strings.HasPrefix(param, "@flow_cursor_value[") {
			...
			var paramIndex, err = strconv.ParseInt(param, 10, 64)
			if err != nil {
				errReturn = fmt.Errorf("internal error: failed to parse parameter name %q", param)
			} else if paramIndex < 0 || paramIndex >= int64(len(argvals)) {
...
func (c *capture) poll(ctx context.Context, binding *bindingInfo) error {
	...
	queryTemplate, err := c.Driver.buildQuery(res, state)
	if err != nil {
		return fmt.Errorf("error building query: %w", err)
	}

	// Expand @flow_cursor_value[N] placeholders to positional ? placeholders,
	// duplicating argument values as needed for multi-column cursors.
	query, args, err := expandQueryPlaceholders(queryTemplate, cursorValues)
	if err != nil {
...
func quoteIdentifier(name string) string {
	// DB2 uses double quotes to delimit identifiers that contain special characters
	// or are reserved keywords. To include a double quote in an identifier, write "".
