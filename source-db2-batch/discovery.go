...
func (drv *BatchSQLDriver) Discover(ctx context.Context, req *pc.Request_Discover) (*pc.Response_Discovered, error) {
	...
	for tableID, table := range tableInfo {
		var resourceName = recommendedCatalogName(table.Schema, table.Name)
...
type discoveredTable struct {
	Schema string
	Name   string
	Type   string // DB2 type code: 'T'=table, 'U'=typed table, 'V'=view, 'W'=typed view
...
func discoverTables(ctx context.Context, db *sql.DB, discoverSchemas []string) ([]*discoveredTable, error) {
	...
	fmt.Fprintf(query, "SELECT TABSCHEMA, TABNAME, TYPE")
	...
	if len(discoverSchemas) > 0 {
		...
		// Exclude DB2 system schemas
		fmt.Fprintf(query, " AND TABSCHEMA NOT LIKE 'SYS%%'")
		fmt.Fprintf(query, " AND TABSCHEMA != 'SYSIBM'")
		fmt.Fprintf(query, " AND TABSCHEMA != 'SYSCAT'")
		fmt.Fprintf(query, " AND TABSCHEMA != 'SYSSTAT'")
		fmt.Fprintf(query, " AND TABSCHEMA != 'SYSFUN'")
		fmt.Fprintf(query, " AND TABSCHEMA != 'SYSPROC'")
		fmt.Fprintf(query, " AND TABSCHEMA != 'SQLJ'")
	}
	...
	for rows.Next() {
		...
		tables = append(tables, &discoveredTable{
			...
			Type:   strings.TrimSpace(tableType),
		})
	}
...
func discoverColumns(ctx context.Context, db *sql.DB, discoverSchemas []string) ([]*discoveredColumn, error) {
	...
	fmt.Fprintf(query, " ORDER BY TABSCHEMA, TABNAME, COLNO")
...
func discoverPrimaryKeys(ctx context.Context, db *sql.DB, discoverSchemas []string) ([]*discoveredPrimaryKey, error) {
	...
	fmt.Fprintf(query, "SELECT TABSCHEMA, TABNAME, COLNAME, COLSEQ")
	fmt.Fprintf(query, " FROM SYSCAT.KEYCOLUSE")
	fmt.Fprintf(query, " WHERE CONSTNAME IN (")
	fmt.Fprintf(query, "   SELECT CONSTNAME FROM SYSCAT.TABCONST WHERE TYPE = 'P'")
	fmt.Fprintf(query, " )")
	if len(discoverSchemas) > 0 {
		fmt.Fprintf(query, " AND TABSCHEMA IN (")
		for i, schema := range discoverSchemas {
	...
	fmt.Fprintf(query, " ORDER BY TABSCHEMA, TABNAME, COLSEQ")
...
var databaseTypeToJSON = map[string]basicColumnType{
	// Boolean Type
	"boolean": {jsonTypes: []string{"boolean"}},

	// Numeric types
	"smallint": {jsonTypes: []string{"integer"}},
	"integer":  {jsonTypes: []string{"integer"}},
	...
	"decfloat": {jsonTypes: []string{"string"}, format: "number"},

	// String types
	"character":  {jsonTypes: []string{"string"}},
	"varchar":    {jsonTypes: []string{"string"}},
	"clob":       {jsonTypes: []string{"string"}},
	"graphic":    {jsonTypes: []string{"string"}},
	"vargraphic": {jsonTypes: []string{"string"}},
	"dbclob":     {jsonTypes: []string{"string"}},
	...
	"blob":      {jsonTypes: []string{"string"}, contentEncoding: "base64"},

	// Date/Time types
	"date":      {jsonTypes: []string{"string"}, format: "date"},
	"time":      {jsonTypes: []string{"string"}}, // Deliberately no format, since DB2 TIME is a bare time without timezone offset
